...
data = pd.read_csv("/Users/ericliao/wnv_prediction_dissertation/national_CDC_code/data_demo/cdc_demo_with_climate.csv",
                   index_col=False,
                   header=0)
...
def svm_model(data, target_column, state):
    ...
    ## add FIPS column
    data_pred["FIPS"] = data_state["FIPS"]
    ...
    ## calculate average of Reported_human_cases group by FIPS from train dataset
    fips_avg = train.groupby("FIPS")["Reported_human_cases"].mean()
    train["Reported_human_cases"] = train["Reported_human_cases"].fillna(train["FIPS"].map(fips_avg))
    test["Reported_human_cases"] = test["Reported_human_cases"].fillna(test["FIPS"].map(fips_avg))
    ...
    print("State: ", state, " mse: ", mse, " rmse: ", rmse, " msle: ", msle, " Q2: ", Q2)

    ## return the mse and q2 score
    return mse, rmse, msle, Q2, num_rows, non_0_num_rows
...
for state in states:
    print("state: ", state)
    mse, rmse, msle, q2, num_rows, non_0_num_rows = svm_model(data, "Reported_human_cases", state)
    state_mse_msle_q2[state] = {"mse": mse, "rmse": rmse, "msle": msle, "q2": q2, "num_rows": num_rows, "non_0_num_rows": non_0_num_rows}
...
rmse = [state_mse_msle_q2[state]["rmse"] for state in state_names]
...
state_mse_rmse_msle_q2_df = pd.DataFrame()
state_mse_rmse_msle_q2_df["State"] = state_names
state_mse_rmse_msle_q2_df["mse"] = mse
state_mse_rmse_msle_q2_df["rmse"] = rmse
state_mse_rmse_msle_q2_df["msle"] = msle
state_mse_rmse_msle_q2_df["q2"] = q2

import plotly.graph_objects as go
from plotly.subplots import make_subplots

# Create a subplot with 2 row and 2 columns
fig = make_subplots(rows=2, cols=1, subplot_titles=("RMSE", "Q2 Score"))
...
fig.add_trace(go.Bar(x=state_names, y=rmse, name='RMSE'), row=1, col=1)
...
fig.update_layout(height=600, width=1000, title_text="RMSE and Q2 Scores by State Using SVM")

fig.write_image("/Users/ericliao/Desktop/WNV_project_files/WNV/CDC_data/human/result/SVM_each_state/human_yearly_cdc_svm_each_state_rmse.png", scale=2)
...
## drop AZ
state_mse_rmse_msle_q2_df = state_mse_rmse_msle_q2_df[state_mse_rmse_msle_q2_df["State"] != "AZ"]

fig2 = go.Figure(data=go.Bar(name='Q2 Score', x=state_mse_rmse_msle_q2_df["State"], y=state_mse_rmse_msle_q2_df["q2"]))

# fig2.update_layout(title_text="Positive Q2 Score by State Using SVM")

fig2.write_image("/Users/ericliao/Desktop/WNV_project_files/WNV/CDC_data/human/result/SVM_each_state/q2_score_svm_each_state.png", scale=2)

## plot mse where only q2 score is positive

fig3 = go.Figure(data=go.Bar(name='RMSE', x=state_mse_rmse_msle_q2_df["State"], y=state_mse_rmse_msle_q2_df["rmse"]))
# fig3.update_layout(title_text="rMSE by State Using SVM with Positive Q2 Score")
fig3.write_image("/Users/ericliao/Desktop/WNV_project_files/WNV/CDC_data/human/result/SVM_each_state/rmse_q2_score_svm_each_state.png", scale=2)


# Create a subplot with 2 row and 2 columns
fig4 = make_subplots(rows=2, cols=1, subplot_titles=( "RMSE ", "Q2 Score"))

## add the number of non_0_num_rowrows to the fourth subplot
fig4.add_trace(go.Bar(x=state_mse_rmse_msle_q2_df["State"], y=state_mse_rmse_msle_q2_df["rmse"]), row=1, col=1)

## add the Positive Q2 Score by State Using SVM
fig4.add_trace(go.Bar(x=state_mse_rmse_msle_q2_df["State"], y=state_mse_rmse_msle_q2_df["q2"]), row=2, col=1)

fig4.update_yaxes(range=[-1, 1], row=2, col=1)

## remove legend
fig4.update_layout(showlegend=False)

# Update y-axis range for the first subplot
# fig4.update_yaxes(title_text="MSE (Log transformed)", row=1, col=1, type='log', tickvals=[1, 10, 100, 1000])  # Example range, adjust as needed

# Update layout if necessary
fig4.update_layout(height=600, width=1000)

fig4.write_image("/Users/ericliao/Desktop/WNV_project_files/WNV/CDC_data/human/result/SVM_each_state/q2_rmse_each_state_non_log.png", scale=2)

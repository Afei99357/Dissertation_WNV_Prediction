...
data = pd.read_csv("/Users/ericliao/Desktop/WNV_project_files/WNV/CDC_data/human/cdc_human_1999_to_2023/"
                 "WNV_human_and_non_human_yearly_climate_demographic_bird_no_impute_missing.csv", index_col=False)
...
def svm_model(data, target_column, state):
    ...
    ## balance the classes
    train = balance_classes(train, target_column)
...
for state in states:
    print("state: ", state)
    mse, msle, q2, num_rows, non_0_num_rows = svm_model(data, "Reported_human_cases", state)
...
fig = make_subplots(rows=2, cols=1, subplot_titles=("MSE", "Q2 Score"))
...
fig.add_trace(go.Bar(x=state_names, y=msle, name='MSE'), row=1, col=1)
...
fig.update_layout(height=600, width=1000, title_text="MSE and Q2 Scores by State Using SVM with Subsampling ")
...
## drop AZ
state_mse_msle_q2_df = state_mse_msle_q2_df[state_mse_msle_q2_df["State"] != "AZ"]
...
# Create a subplot with 2 row and 2 columns
fig4 = make_subplots(rows=2, cols=1, subplot_titles=( "MSE ", "Q2 Score"))

## add the number of non_0_num_rowrows to the fourth subplot
fig4.add_trace(go.Bar(x=state_mse_msle_q2_df["State"], y=state_mse_msle_q2_df["mse"]), row=1, col=1)
...
fig4.write_image("/Users/ericliao/Desktop/WNV_project_files/WNV/CDC_data/human/result/SVM_each_state_subsampling/positive_q2_mse_each_state_subsampling.png", scale=2)
